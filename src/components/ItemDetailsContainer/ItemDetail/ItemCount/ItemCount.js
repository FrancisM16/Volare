...
export const ItemCount = ({ stock, amount, setAmount, handleAdd }) => {
    ...
    if (stock === 0) {
        return (
            <p className="text-violet-900">No hay stock disponible</p>
        )
    }
    ...
    return (
        <div className="container mx-auto">
            <div className="flex flex-col mt-4 space-y-4 md:space-y-0 md:space-x-8 md:items-center md:flex-row font-default">
                ...
                <button onClick={handleAdd} className="border rounded-md py-2 px-4 text-white bg-violet-900 hover:bg-black md:w-fit w-full">Agregar al carrito</button>
            </div>
        </div>
    )
}