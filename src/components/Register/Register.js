...
export const Register = () => {
	...
	const { registerUser, registerUserInfo, showModalError } =
	...
	const onSubmit = async data => {
		try {
			...
			await registerUserInfo(data, userCredential.user.uid);
			cart.length === 0 ? navigate('/') : navigate('/cart');
		} catch (e) {
			navigate('/register');
			showModalError('Correo ya en uso');
		}
	};
	...
	return (
		<div className='flex w-full h-screen'>
			<div className='w-full flex items-center justify-center lg:w-1/2'>
				<div className='w-11/12 max-w-[700px] px-10 py-20 rounded-3xl bg-white border-2 border-gray-100 my-4'>
						...
						<form className='space-y-6' onSubmit={handleSubmit(onSubmit)}>
								...
								{errors.phone && (
									<div className='space-x-2 text-red-600'>
										<FontAwesomeIcon
											...
											size='xs'
										/>
										<span className='text-sm'>Teléfono es obligatorio</span>
									</div>
								)}
							</label>
								...
								<input
									...
									{...register('confirmEmail', {
										...
										validate: val => {
											...
											if (watch('email') !== val) {
												return 'Correos electrónicos deben ser iguales';
											}
										},
									})}
								/>
							...
							<div className='grid justify-items-center space-x-0 space-y-6 md:flex md:items-center md:space-y-0 md:space-x-4'>
								...
								<div className='text-center'>
									<Link to='/login' className=' hover:text-violet-900 m-4'>
